#!/bin/bash
# this runs the BoE example as a local instace of instance of aca-py
...
PYTHONPATH=.. ../bin/aca-py start \
   ...
   --label beeds_user.agent \
   --inbound-transport http 0.0.0.0 8030 \
   --outbound-transport http \
   --admin 0.0.0.0 8031 \
   --admin-insecure-mode \
   --wallet-type indy \
   --wallet-name beeds_user.agent420695 \
   --wallet-key beeds_user.agent420695 \
   --preserve-exchange-records \
   --auto-provision \
   --genesis-url http://localhost:9000/genesis \
   --trace-target log \
   --trace-tag acapy.events \
   --trace-label beeds_user.agent.trace \
